...
export default {
    ...
    getSuggestions(suggestions){
        if ((!suggestions.prefix.length) && (!suggestions.activatedManually)) return;
        ...
        for (type of scopes)
        {
            if (type.startsWith("meta.entity.name.function"))
            {
                ...
                {
                    if (this.api.functions != undefined)
                    {
                        let newScope = this.api.functions.find((e) => e.name === func);
                        if (newScope != undefined) scope.push(newScope);
                    }
                }
            }
        ...
        for (context of scope.reverse())
        {
            ...
            {
                if ((context.name != undefined) && (prefixType.endsWith(`blob-function-${context.name}.blob`)))
                {
